...
namespace Timesheet.Infrastruture.ReadModel.Queries
{
    public class QueryReferential : IQueryReferential
    {
        ...
        public async Task<IEnumerable<PayrollPeriod>> GetPayrollPeriods(string type = "hourly")
        {
            ...
            string endDateQry = type == "hourly" ? "DATEADD(DAY, 5 - DATEPART(WEEKDAY,  DATEADD(YEAR, 2, GETDATE())), CAST(DATEADD(YEAR, 2, GETDATE()) AS DATE))" : (DateTime.Now.Day > 15 ? "DATEADD(YEAR,2,EOMONTH(GETDATE()))" : "DATEADD(YEAR,2,DATEADD(DAY, 14, DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)))");
