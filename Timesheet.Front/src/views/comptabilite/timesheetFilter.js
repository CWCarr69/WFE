  ...
  const fetchPayrollPeriods = async (type) => {
    ...
    await getPayrollPeriods(type)
      .then((resp) => {
        ...
        // Find the current period where today is between start and end
        const today = new Date();
        const currentPeriod = adaptedPeriods.find(period => {
          const startDate = new Date(period.start);
          const endDate = new Date(period.end);
          return today >= startDate && today <= endDate;
        });

        // Use current period if found, otherwise fall back to first period
        if (currentPeriod) {
          console.log('Found current period:', currentPeriod);
          setSelectedPeriod(currentPeriod);
          onPayrollPeriodChanged && onPayrollPeriodChanged(currentPeriod.value);
        } else if (adaptedPeriods?.length > 0) {
          console.log('No current period found, using first period');
          setSelectedPeriod(adaptedPeriods[0]);
          onPayrollPeriodChanged && onPayrollPeriodChanged(adaptedPeriods[0].value);
        }
      })
